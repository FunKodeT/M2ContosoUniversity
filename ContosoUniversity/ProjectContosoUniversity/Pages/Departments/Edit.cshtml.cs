...
namespace ProjectContosoUniversity.Pages.Departments
{
    public class EditModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(int id)
        {
            ...
            Department = await _context.Departments.Include(d => d.Administrator).AsNoTracking().FirstOrDefaultAsync(m => m.DepartmentID == id);
        ...
        public async Task<IActionResult> OnPostAsync(int id)
        {
            ...
            // Fetch current department from DB. ConcurrencyToken may have changed.
            ...
            if (await TryUpdateModelAsync<Department>(departmentToUpdate, "Department", s => s.Name, s => s.StartDate, s => s.Budget, s => s.InstructorID))
            {
                ...
                catch (DbUpdateConcurrencyException ex)
                {
                    ...
                    // Save the current ConcurrencyToken so next postback matches, unless a new concurrency issue happens.
        ...
        {
            // ModelState contains the posted data because of the deletion error and overrides the Department instance values when displaying Page().
            ModelState.AddModelError(string.Empty, "Unable to save. The department was deleted by another user.");
            InstructorNameSL = new SelectList(_context.Instructors, "ID", "FullName", Department.InstructorID);
            return Page();
        }

        private async Task SetDbErrorMessage(Department dbValues, Department clientValues, StudentContext context)
        {
            ...
            ModelState.AddModelError(string.Empty, "The record you attempted to edit " + "was modified by another user after you. " + "edit operation was canceled and the current values in the database " + "have been displayed. If you still want to edit this record, click " + "the Save button again.");
        }
    }
}
